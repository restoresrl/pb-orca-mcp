...
**Compile-loop validation via MCP — done** (2026-05-13). Eseguito end-to-end
con Claude Code in driver dopo il riavvio post-`a8f9756`: `pb_session_open`
→ `pb_set_library_list` → `pb_set_current_application` →
`pb_library_entry_export` → edit + prepend `$PBExportHeader$<name>.<ext>`
→ `pb_compile_entry_import` (success=True, errors=[]) →
`pb_library_entry_export` (modifica persistita) → `pb_application_rebuild`
(incremental, success) → `pb_session_close`. Tutto l'happy path coperto
sia da pytest sia da MCP. v0.1.0 è pronto per il tag.
