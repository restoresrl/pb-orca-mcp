...
IDE would show, line and column included. The agent correlates `message_text`
...
pushes the derived form back onto the SOT, the wrong direction).
...
The native ORCA SCC flow collapses steps 4 and 5 into one call sequence; see
