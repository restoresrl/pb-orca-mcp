...
3. `pb_set_current_application`: required before any **compile**,
   **rebuild**, or **object-query** call. **May rewrite the `.pbw`** as a
   side effect, so revert it afterwards unless you changed the target list.
4. ... do the work ...
5. `pb_session_close`.

Switching PB version means close + reopen (a process holds one PB runtime).
...
- `pb_application_rebuild`: `full` / `incremental` / `migrate` / `3pass`
  rebuild of the current application. Needs steps 1 to 3 of the lifecycle done.
